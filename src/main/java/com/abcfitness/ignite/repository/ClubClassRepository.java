...
public interface ClubClassRepository extends JpaRepository<ClubClass, Long> {
    ...
    @Query("select count(cc)>0 from ClubClass cc join Club c on cc.club.id=c.id  where c.id=:clubId and (( :startDate>=cc.startDate and :startDate<=cc.endDate ) or ( :endDate>=cc.startDate and :endDate<=cc.endDate) or ( :startDate<=cc.startDate and :endDate>=cc.endDate ))")
